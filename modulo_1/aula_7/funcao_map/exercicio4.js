/* 
O objeto const people é formado pelo nome (name), idade (age), peso (weight) e altura (height) de 5 pessoas.
...
const pessoas = people.map(pessoa => {
    return{
        ...pessoa,
        bmi: parseFloat((pessoa.weight / (pessoa.height ** 2)).toFixed(1)),
    }
})
...
const todasAsPessoas = pessoas.map(pessoa => {
    return{
        ...pessoa,
        classification: (pessoa.bmi >= 18.5 && pessoa.bmi < 25) ? "IMC Normal" : "Fora da faixa normal"
    }
})
console.table(todasAsPessoas)